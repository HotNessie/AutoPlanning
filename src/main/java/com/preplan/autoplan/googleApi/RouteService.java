...
public class RouteService {
  ...
  public ComputeRoutesResponse computeRoutes(ComputeRoutesRequest request) {
    ...
    for (int i = 0; i < places.size() - 1; i++) {
      ...
      if (mode == Transport.TRANSIT) {
        GoogleRoutesRequest apiRequest = createTransitRequest(start, places.get(i + 1), currentDeparture, request);
        ...
        GoogleRoutesRequest apiRequest = createDriveRequest(start, places.get(j), intermediates, currentDeparture,
            request);
        ComputeRoutesResponse response = googleRouteClient.getRoute(routeFieldMask, apiRequest);
        responses.add(response);
        currentDeparture = updateDepartureTime(response, currentDeparture, start.time());
  ...
  private GoogleRoutesRequest createTransitRequest(
    ...
    return createRequest(origin, destination, null, Transport.TRANSIT, departureTime, originalRequest);
  }
  ...
  private GoogleRoutesRequest createDriveRequest(
    ...
    List<GoogleRoutesRequest.Place> intermediatePlaces = intermediates.stream()
        .map(this::toPlace)
        .toList();
    return createRequest(origin, destination, intermediatePlaces, Transport.DRIVE, departureTime, originalRequest);
  }
  ...
  // 출발 시간 업데이트
  private LocalDateTime updateDepartureTime(ComputeRoutesResponse response, LocalDateTime currentDeparture,
      Integer stayTime) {
