...
export function initPlanContent() {
  ...
  currentPlanData = {
    ...
    departureTime: planResponseDto.departureTime || null,
...
function proceedToSavePlan() {
  ...
  */
  ...
  // 사용자가 제목을 수정하지 않은 경우, null로 설정
  if (finalTitle.replace(/\s/g, '') === '여행계획완성') {
    finalTitle = null;
  }
  ...
  */
  const routes = currentPlanData.places.map((place, index) => {
    ...
    let transitDetailInfo = null;

    if (place.transit === 'TRANSIT' && currentLeg.steps) {
      transitDetailInfo = (currentLeg) => {
        if (!currentLeg.steps) return null;
        const transitInfo = currentLeg.steps
          .filter(step => step.transitDetails && step.transitDetails.transitLine)
          .map(step => {
            const line = step.transitDetails.transitLine;
            const name = line.name || line.shortName || '알 수 없음';
            const shortName = line.shortName || '';

            if (name) {
              if (shortName && !name.includes(shortName)) {
    ...
    return {
      ...
      transitDetailInfo: currentLeg.transitDetailInfo || '',
    }
  });
...
function handleEditPlan() {
  const loadContent = async (url) => {
    try {
      ...
      if (url === '/selfContent') {
        const { initSelfContent, initRouteFormHandler, getDynamicElements } = await import('../selfContent/selfContent.js');
